...
public class InputOutputCoEfficientController : ControllerBase
{
    ...
    public InputOutputCoEfficientController(IPriceSetupProductCostService productCost,
    ...
    public async Task<IActionResult> Create()
    {
        ...
        var productGroupDT = await _productGrpService.Query().Where(x => x.OrganizationId == UserSession.OrganizationId).SelectAsync();
        var productTypeDT = await _productTypeService.Query().SelectAsync();

        var productCat = await _productCategoryService.Query().Where(x => x.OrganizationId == UserSession.OrganizationId).SelectAsync();

        var productVatType = await _productVatTypeService.Query().SelectAsync();

        IEnumerable<CustomSelectListItem> ProductCategories = productCat
            .Where(x => x.OrganizationId == UserSession.OrganizationId).Select(s => new CustomSelectListItem
            {
                Id = s.ProductCategoryId,
                Name = s.Name
            });

        IEnumerable<CustomSelectListItem> ProductGroups = productGroupDT.Select(s => new CustomSelectListItem
        {
            Id = s.ProductGroupId,
            Name = s.Name
        });
        ...
        IEnumerable<CustomSelectListItem> ProductTypes = productTypeDT.Select(s => new CustomSelectListItem
        {
            Id = s.ProductTypeId,
            Name = s.Name
        });

        //IEnumerable<SelectListItems> ProductVat = productVatType.Select(s => new SelectListItems
        //{
        ...
        {
            ProductTypeSelectListItems = ProductTypes,
            MeasurementUnits = MeasurementUnits,
            ProductCategories = await _productCategoryService.GetProductCategorySelectList(UserSession.ProtectedOrganizationId),
    ...
    public async Task<IActionResult> Create(vmProduct product)
    {
        ...
        var productCat = await _productCategoryService.Query().Where(x => x.OrganizationId == UserSession.OrganizationId).SelectAsync();
        IEnumerable<CustomSelectListItem> ProductCategories = productCat.Select(s => new CustomSelectListItem
        {
            Id = s.ProductCategoryId,
            Name = s.Name
        });
        IEnumerable<CustomSelectListItem> ProductVattypes = productVatTypeDT.Select(s => new CustomSelectListItem
        {
            Id = s.ProductVattypeId,
            Name = s.Name
        });
        ...
        product.MeasurementUnits = MeasurementUnits;
        product.ProductGroups = ProductGroups;
        product.ProductVattypes = ProductVattypes;
        product.ProductCategories =
            await _productCategoryService.GetProductCategorySelectList(UserSession.ProtectedOrganizationId);
    ...
    [VmsAuthorize(FeatureList.PRODUCT_PRODUCT_LIST_CAN_VIEW)]
    [VmsAuthorize(FeatureList.PRODUCT_PRODUCT_LIST_CAN_EDIT)]
    public async Task<IActionResult> Edit(vmProduct product, string id)
    {
