...
const commonUtilityIOC = {
    ...
    getTotalFinal: () => {
        ...
        return (totalOverhead + totalProductCost);
    },
    getProfitToPercent: () => {
...
$(document).on('input', '#tbdOverheadCost', function (event) {
    
    debugger;
    function calculateTotaOverHeadlValue() {
        var total = 0;
        var profitAmt = parseFloat($('.ovhdProfit').val()) || 0;
       $('.ovhdcost').each(function () {
            var value = parseFloat($(this).val()) || 0;
            total += value;
        });
        $('#totalOverHeadCost').text(total + profitAmt); 
    }
...
function handleInputChangeInReqQt(event) {
    ...
    const existingEntryIndex = listInfo.productCost.findIndex(item => item.RndId);// === idToReplace);

    const filteredProducts = listInfo.productCost.filter(item => item.ProductId !== undefined && item.ProductId !== null);
    const ProductIdArray = filteredProducts.map(item => item.ProductId);
    const ProductId = ProductIdArray.join(',');

    const filteredMeasurementUnitId = listInfo.productCost.filter(item => item.MeasurementUnitId !== undefined && item.MeasurementUnitId !== null);
    const MeasurementUnitIdArray = filteredMeasurementUnitId.map(item => item.MeasurementUnitId);
    const MeasurementUnitId = MeasurementUnitIdArray.join(',');

    const filteredWastagePercentage = listInfo.productCost.filter(item => item.WastagePercentage !== undefined && item.WastagePercentage !== null);
    const filteredWastagePercentageArray = filteredWastagePercentage.map(item => item.WastagePercentage);
    const WastagePercentage = filteredWastagePercentageArray.join(',');

    if (existingEntryIndex !== -1) {
        listInfo.productCost[existingEntryIndex] = {
            RequireQty: RequireQty,
            ProductCost: RequireQty * UnitPrice,
            UnitPrice: UnitPrice,
...
function handleInputChangeInProductCost(event) {
    ...
    const row = event.target.closest('tr');
    const productCost = parseFloat(row.querySelector('[id^="txtProductCost_"]').value) || 0;
    const indexToUpdate = 0; // Replace this with the appropriate index
    // Update 'ProductCost' property of the object at 'indexToUpdate'
    if (indexToUpdate >= 0 && indexToUpdate < listInfo.productCost.length) {
        listInfo.productCost[indexToUpdate].ProductCost = productCost;
    }
...
const sendToInputOutputCoEfficientController = () => {
    ...
    //if (window.generalUtility.validateElement.formByForm(elementInformation.formInfo.frmProfit)) {
        ...
        var ProfitAmount = $('.ovhdProfit').val();
   ...
   // listInfo.productCost
    ...
    for (const relatedOverheadCost of listInfo.dataListOverHead) {
        if (relatedOverheadCost.OverheadCostAmount > 0 || relatedOverheadCost.ProfitAmount>0) {
