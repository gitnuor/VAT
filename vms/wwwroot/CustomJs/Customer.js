...
classesToShowHideInEvents.customerRelatedList =
...
mainFormFields = {
    ...
    isForeignCustomer: document.getElementById('IsForeignCustomer'),
...
const dropDownFormFields = {
    ...
    ddlDeliveryCountry: document.getElementById('DeliveryCountryId'),
    ddlDeliveryDivision: document.getElementById('DeliveryDivisionOrStateId'),
    ddlDeliveryDistrict: document.getElementById('DeliveryDistrictOrCityId')
}

const commonChangeEventsForDisplay = {
    hidecustomerSpecialOptions: () => {
        classesToShowHideInEvents.customerRelatedList.forEach(element => {
            window.generalUtility.displayOption.hideItemByClassName(element);
        });
    },
    displayForeign: () => {
        commonChangeEventsForDisplay.hidecustomerSpecialOptions();
...
const DropDownListMaker = (toId, name, Routid, url) => {
    $(toId).val = '';
    var s = '<option value="">' + name + '</option>';
    var link = location.origin + url + Routid;
    $(toId).html(s);
    $.ajax({
        type: "GET",
        url: link,
...
dropDownFormFields.ddlCountry.addEventListener('change',
    () => {
        let countryId = dropDownFormFields.ddlCountry.value;
...
dropDownFormFields.ddlDivision.addEventListener('change',
    () => {
        let divisionId = dropDownFormFields.ddlDivision.value;
...
//dropDownFormFields.ddlDeliveryCountry.addEventListener('change', () => {
//    let countryId = dropDownFormFields.ddlDeliveryCountry.value;
//    DropDownListMaker(dropDownFormFields.ddlDeliveryDivision, 'Select Division', parseInt(countryId), '/DivisionOrState/GetDivisionsByCountryId/');
//});

//dropDownFormFields.ddlDeliveryDivision.addEventListener('change', () => {
//    let divisionId = dropDownFormFields.ddlDeliveryDivision.value;
//    DropDownListMaker(dropDownFormFields.ddlDeliveryDistrict, 'Select District', parseInt(divisionId), '/DistrictOrCity/GetDistrictsByDivisionId/');
//});

dropDownFormFields.ddlBankBranchCountry.addEventListener('change',
    () => {
        let countryId = dropDownFormFields.ddlBankBranchCountry.value;
...
mainFormFields.isForeignCustomer.addEventListener('change',
    () => {
        if (mainFormFields.isForeignCustomer.checked) {
