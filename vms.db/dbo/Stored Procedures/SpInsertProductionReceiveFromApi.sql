...
-- =============================================
CREATE PROCEDURE [dbo].[SpInsertProductionReceiveFromApi]
    @ProductionReceiveId NVARCHAR(100),
    @BatchNo NVARCHAR(50),
    @OrganizationId INT,
    @ProductionId INT,
    ...
    @IsContractual BIT,
    @ContractualProductionId NVARCHAR(100),
...
BEGIN
    ...
    BEGIN TRY
        ...
        DECLARE @NoOfRawMaterialExceedStock INT = 0,
                @ApiTransactionTypeId INT = 5;
        DECLARE @ApiTransactionId BIGINT,
                @MainProductId INT,
                @MainMeasurementUnitId INT,
                @MainContractualProductionId INT;
        SELECT @MainProductId = prod.ProductId
        FROM dbo.Products prod
        WHERE prod.ReferenceKey = @ProductId
              AND prod.OrganizationId = @OrganizationId;
        SELECT @MainMeasurementUnitId = mu.MeasurementUnitId
        FROM dbo.MeasurementUnits mu
        WHERE mu.ReferenceKey = @MeasurementUnitId
              AND mu.OrganizationId = @OrganizationId;
        SELECT @MainContractualProductionId = conProd.ContractualProductionId
        FROM dbo.ContractualProduction conProd
        WHERE conProd.ReferenceKey = @ContractualProductionId;
        ...
        WHERE ps.ProductId = @MainProductId
        ...
        (   @BatchNo,                        -- BatchNo - nvarchar(50)
            ...
            @MainMeasurementUnitId,          -- MeasurementUnitId - int
            @ReceiveTime,                    -- ReceiveTime - datetime
            @MaterialCost,                   -- MaterialCost - decimal(18, 2)
            1,                               -- IsActive - bit
            @ProductionReceiveId,            -- ReferenceKey - nvarchar(100)
            @CreatedBy,                      -- CreatedBy - int
            @CreatedTime,                    -- CreatedTime - datetime
            @IsContractual,                  -- IsContractual - bit
            @MainContractualProductionId,    -- ContractualProductionId - int
        ...
        SET @MainProductionReceiveId = SCOPE_IDENTITY();
        ...
        WHERE ptb.OrganizationId = @OrganizationId
              AND ptb.ProductId = @MainProductId
        ...
        (   @OrganizationId,                          -- OrganizationId - int
            ...
            @MainProductionReceiveId,                 -- ProductionReceiveId - bigint
        ...
        SELECT @MainProductionReceiveId,    -- ProductionReceiveId - bigint
            ...
            (
                ...
                FROM dbo.ProductTransactionBook ptb
                    ...
                    (
                        ...
                        WHERE ptb.OrganizationId = @OrganizationId
                              AND bomToCompare.ProductionReceiveId = @MainProductionReceiveId
                        GROUP BY ptb.ProductId
                    ) lastPtb
                        ON lastPtb.ProductTransactionBookId = ptb.ProductTransactionBookId
                    CROSS APPLY
        ...
        WHERE bom.ProductionReceiveId = @MainProductionReceiveId;
        --Insert used Raw Material in ProductTransactionBook
        ...
        VALUES
        (   33,                       -- ObjectTypeId - int
            @MainProductionReceiveId, -- PrimaryKey - int
