...
-- =============================================
CREATE PROCEDURE [dbo].[SpInsertSaleFromApi]
...
BEGIN
    BEGIN TRY
        ...
        DECLARE @OrganizationStaticDataTypeVatType INT = 14,
                @OrganizationStaticDataTypePaymentMethod INT = 13;
        ...
        BEGIN
            ...
            WHERE (
                      osd.OrgStaticDataTypeId = @OrganizationStaticDataTypePaymentMethod
        ...
        WHERE (
                  osd.OrgStaticDataTypeId = @OrganizationStaticDataTypeVatType
                  AND osd.OrganizationId = @OrganizationId
                  AND osd.IsActive = 1
              )
              OR osd.OrgStaticDataId IS NULL;
        ...
        (   @InvoiceNo,                        -- InvoiceNo - nvarchar(50)
            ...
            @SalesId,                          -- ReferenceKey - nvarchar(100)
        ...
        --Get SalesId
        SET @MainSalesId = SCOPE_IDENTITY();
        ...
        SELECT @MainSalesId,                -- SalesId - int
            ...
            (
                ...
                FROM dbo.ProductTransactionBook ptb
                    ...
                    (
                        ...
                        WHERE ptb.OrganizationId = @OrganizationId
                              AND slsDtlToCompare.SalesId = @MainSalesId
                        GROUP BY ptb.ProductId
                    ) lastPtb
                        ON lastPtb.ProductTransactionBookId = ptb.ProductTransactionBookId
                    CROSS APPLY
        ...
        WHERE slsDtl.SalesId = @MainSalesId;
        --Insert sales details in ProductTransactionBook
